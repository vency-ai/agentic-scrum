...
def main():
    ...
    # 3. Group tasks by employee and construct the consolidated report payload
    logging.info("Step 3: Constructing consolidated daily scrum report...")
    ...
    for task in tasks:
        ...
        # Determine task status and construct descriptive messages
        task_title = task.get('title', 'N/A')
        progress_percentage = task.get('progress_percentage', 0)

        if progress_percentage == 100:
            yesterday_work_msg = f"Completed task: '{task_title}'."
            today_work_msg = "No further work on this task."
        elif progress_percentage > 0 and progress_percentage < 100:
            yesterday_work_msg = f"Worked on task: '{task_title}', currently {progress_percentage}% complete."
            today_work_msg = f"Continuing work on task: '{task_title}'."
        else: # progress_percentage == 0 or status indicates open/not started
            yesterday_work_msg = f"No progress on task: '{task_title}'."
            today_work_msg = f"Starting work on task: '{task_title}'."

        if employee_id not in employee_reports:
            employee_reports[employee_id] = {
                "assigned_to": employee_id,
                "tasks": []
            }
        employee_reports[employee_id]["tasks"].append({
            "id": task_id,
            "yesterday_work": yesterday_work_msg,
            "today_work": today_work_msg,
            "impediments": "None reported.",
            "created_at": datetime.now(timezone.utc).isoformat()
        })

    # Convert employee_reports dictionary to a list of reports for the payload
    reports_list = list(employee_reports.values())

    # Construct the final payload for the daily scrum report
    daily_scrum_report_payload = {
        "project_id": PROJECT_ID,
        "sprint_id": SPRINT_ID,
        "report_date": report_date,
        "summary": f"Daily stand-up for sprint {SPRINT_ID}. {len(tasks)} tasks had progress simulated.",
        "summary_metrics": summary_metrics,
        "reports": {
            report_date: reports_list
        }
    }

    logging.info(f"Posting consolidated daily scrum report for sprint {SPRINT_ID}")
    create_daily_scrum_report(daily_scrum_report_payload)
